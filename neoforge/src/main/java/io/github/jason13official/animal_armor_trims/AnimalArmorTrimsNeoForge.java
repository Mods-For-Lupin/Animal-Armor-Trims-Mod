...
public class AnimalArmorTrimsNeoForge {
  ...
  public AnimalArmorTrimsNeoForge(final IEventBus modEventBus) {
    ...
    EVENT_BUS.addListener((Consumer<FMLCommonSetupEvent>) event -> AnimalArmorTrims.init());

    NeoForge.EVENT_BUS.addListener((Consumer<AddServerReloadListenersEvent>) event -> {
      event.addListener(AnimalArmorTrims.identifier(Constants.MOD_ID), new ResourceReloadListener());
    });

    if (FMLLoader.getCurrent().getDist() == Dist.CLIENT) {
      new AnimalArmorTrimsClientNeoForge(EVENT_BUS);
    }
  }
  ...
  public static class ResourceReloadListener extends SimplePreparableReloadListener<Void> {
    ...
    public String getName() {
      return AnimalArmorTrims.identifier(Constants.MOD_ID).toString();
    }
